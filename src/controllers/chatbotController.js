...
const funcA = async (req, res) => {
   console.log('FuncA')
   setTimeout(() => {
      axios.get(req.protocol + '://' + req.get('host') + '/funcB')
   }, 5000)
}
const startLoop = async (req, res) => {
   axios.get(req.protocol + '://' + req.get('host') + '/funcA')
}
const funcB = async (req, res) => {
   console.log('FuncB')
   setTimeout(() => {
      axios.get(req.protocol + '://' + req.get('host') + '/funcA')
   }, 5000)
...
export default { getHomePage, getWebHook, postWebHook, startLoop, funcA, funcB, sayHello }
